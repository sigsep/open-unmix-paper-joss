...
Music separation is a problem which fascinated researchers for over 50 years. This is partly because, mathematically, there exists no closed-form solution when many sources (instruments) are captured by a single microphone.
As the problem is impossible to solve without some assumptions when there are fewer mixtures than sources, researchers exploited further knowledge about the way the signals were recorded and mixed to make separation possible. A large number of these methods are centered around "classical" signal processing methods that revolve around assumptions concerning the sources to separate. For a more detailed overview see [@rafii17] and [@cano19].
...
Many new methods that were published in the last years, are usually compared to their baselines, thus showing relative instead of absolute performance gains, so that other researchers cannot assess if a method performs as good as state-of-the-art.
...
- __simple to extend__: the pre/post-processing, data-loading, training and models part of the code is isolated and easy to replace/update. In particular, an extra-effort was done on making it easy to replace the core deep model that is currently a baseline BLSTM.
...
For training of the recurrent layers, we use sequences that correspond to six seconds duration and use 16 samples per minibatch.
